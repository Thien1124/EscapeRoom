...
VALUES
    ('Tủ gỗ khóa mã 3 số', 'Dùng bút chì tô giấy trắng để lộ mã', 68, 33, TRUE, 1, 'QUIZ', (SELECT id FROM game_rooms WHERE name = 'Thí Nghiệm Bị Ngắt Quãng' LIMIT 1)),
    ('Kính hiển vi', 'Lắp ống kính và mẫu vật để soi', 40, 38, TRUE, 2, 'INTERACTION', (SELECT id FROM game_rooms WHERE name = 'Thí Nghiệm Bị Ngắt Quãng' LIMIT 1)),
    ('Máy quét cạnh cửa', 'Đổ dung dịch tím vào phễu để kích hoạt', 78, 45, TRUE, 3, 'INTERACTION', (SELECT id FROM game_rooms WHERE name = 'Thí Nghiệm Bị Ngắt Quãng' LIMIT 1)),
    ('Bảng nhập hình dạng vi khuẩn', 'Nhập hình dạng quan sát được từ kính hiển vi', 82, 28, TRUE, 4, 'QUIZ', (SELECT id FROM game_rooms WHERE name = 'Thí Nghiệm Bị Ngắt Quãng' LIMIT 1));
